...
export interface IStorage {
  ...
  // User Car Preferences operations
  ...
  // Get cars by IDs for recommendations
...
export class DatabaseStorage implements IStorage {
  ...
  async syncExternalCar(id: number, car: InsertCar): Promise<void> {
    ...
    if (existing) {
      ...
      // Insert with specific ID using raw SQL to override serial
      await db.execute(
  ...
  async getMarketingBanners(position?: string): Promise<MarketingBanner[]> {
    let query = db.select().from(marketingBanners).where(eq(marketingBanners.isActive, true)).orderBy(desc(marketingBanners.priority));
    if (position) {
      query = query.where(eq(marketingBanners.position, position)) as any;
    }
    return query;
  }
