...
type Props = Pick<SellStepsProps, "formData" | "vehicleData" | "searchQuery" | "setSearchQuery" | "modelsLoading" | "filteredModels" | "selectModel">;

export function ModelStep({ formData, vehicleData, searchQuery, setSearchQuery, modelsLoading, filteredModels, selectModel }: Props) {
  const [modelDropdownOpen, setModelDropdownOpen] = useState(!formData.model);

  React.useEffect(() => {
    setModelDropdownOpen(!formData.model);
  }, []);
