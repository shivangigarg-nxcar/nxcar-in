...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetchWithRetry(
      `${BASE_URL}/userprofile-mycars-sell-ads`,
      {
        method: 'POST',
        headers,
    ...
    if (!response.ok) {
      console.error('mycars-sell-ads API error:', response.status);
      return NextResponse.json([], { status: 200 });
    }

    if (!contentType.includes('application/json')) {
      console.error('mycars-sell-ads returned non-JSON response:', contentType);
      return NextResponse.json([], { status: 200 });
    }
    ...
    try {
      ...
      console.error('mycars-sell-ads JSON parse error:', parseErr);
      return NextResponse.json([], { status: 200 });
    }
    ...
    console.error('mycars-sell-ads error:', error);
    return NextResponse.json([], { status: 200 });
  }
}
