...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetchWithRetry(
      `${BASE_URL}/userprofile-mycars-buy`,
      {
        method: 'POST',
        headers,
    ...
    if (!response.ok) {
      console.error('mycars-buy API error:', response.status);
      return NextResponse.json([], { status: 200 });
    }

    if (!contentType.includes('application/json')) {
      console.error('mycars-buy returned non-JSON response:', contentType);
      return NextResponse.json([], { status: 200 });
    }
    ...
    try {
      ...
      console.error('mycars-buy JSON parse error:', parseErr);
      return NextResponse.json([], { status: 200 });
    }
    ...
    console.error('mycars-buy error:', error);
    return NextResponse.json([], { status: 200 });
  }
}
