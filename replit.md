...
- **Book Inspection Modal**: `BookInspectionModal` in `app/my-cars/page.tsx` opens when clicking "Book Inspection" on SellCarCard. Collects city, date, time slot (Morning/Afternoon/Evening from `/api/nxcar/inspection-slots`), pincode, address. Cities from `/api/sell-cities` (returns `{ success, cities }` format). Submits to `POST /api/book-inspection`. Date handling uses `toLocalDateString()` helper (not `toISOString()`) to avoid timezone off-by-one bugs. Time slots are disabled when <2 hours from now (today only) via `isSlotTooSoon()`. After successful booking, shows "Next Step: Upload Documents" prompt with "Upload Now" (redirects to `/upload-documents?vehicle_id=X`) and "I'll Do It Later" (closes modal).
- **Inspection Booking (Sell Flow)**: `inspection-booking.tsx` includes same `toLocalDateString()` and `isSlotTooSoon()` fixes. Home inspection mode has mandatory 6-digit PIN code field (`inspectionPincode` state in `use-sell-car-form.ts`), threaded through `sell-success-section.tsx` → `inspection-booking.tsx` → `franchise-list.tsx`. Book button disabled if PIN invalid. PIN sent explicitly in API payload instead of extracting from address.
- **Document Upload Page**: `app/upload-documents/page.tsx` — dedicated full-page upload experience with `?vehicle_id=X` query param. Individual upload slots for: RC front/back, Insurance, PAN Card, Aadhaar front/back. Each slot has file picker (image/PDF), preview thumbnail, confirm/remove buttons, green check after confirmed. Payment section with bank account (account number, IFSC, holder name) or cancelled cheque toggle. Uploads via `/api/nxcar/image-upload` endpoint. Replaces old `DocumentUploadSection` as primary upload flow.
- **Document Upload (Legacy)**: `DocumentUploadSection` in car detail page still exists for backward compatibility when `?from=sell` query param is present.
...
- **Scroll Restoration**: `ScrollRestoration` in `components/scroll-restoration.tsx` (wrapped in Suspense, mounted in `providers.tsx`) saves scroll positions per route (pathname + search params) to sessionStorage. Detects back navigation via `popstate` event with history length comparison. Blocks Next.js zero-scroll resets by patching `window.scrollTo`/`scroll` during a 4s restoration window. Restores via `document.documentElement.scrollTop` with retries. All timers are cleaned up on route change.
- **Fair Range in Sidebar**: `seller-action-sidebar.tsx` accepts optional `priceMap` prop and displays Nxcar Fair Range section with gradient strip matching production nxcar.in (coins icon + price range + "Recommended Price By Nxcar." badge). Validates `buyerUpper > buyerLower > 0` before rendering.
- **Listing Price Chart**: `listing-price-chart.tsx` renders a D3 scatter chart on car detail pages showing listing prices by year with km distance spread (0-150K km). Uses `use-car-chart-data.ts` hook to fetch data from `/api/nxcar/fetch-car-data` (proxies to `ai.nxcar.in/fetch-car-data`). Features: city-colored car icon dots, current car highlighted with teal glow, region toggle buttons, clickable bars open detailed year view modal. Chart hides gracefully when API returns insufficient data (<5 listings).
